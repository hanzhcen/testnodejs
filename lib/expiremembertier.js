...
module.exports = {
    expiremembertier:function(pool, processDate,environment){
        return expiremembertier (pool, processDate,environment);
    },
    
};

async function expiremembertier (pool, processDate,environment){
    try{
        ...
        for(var i=0; i<member_list.length;i++){
            try{
                ...
                querystring = "SELECT SUM(t.ivls__pointsearned__c) as pointsearned FROM loyaltycore.ivls__transaction__c as t JOIN loyaltycore.account as a ON t.ivls__memberpgid__c = a.ivls__pgid__c WHERE t.ivls__memberpgid__c = "+oldMemberObj.ivls__pgid__c+" AND t.ivls__transactiondate__c > a.ivls__datetiertime__c"
