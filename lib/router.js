...
if(environment == 'production'){
    ...
    var config = {
        ...
        max: 10, // max number of clients in the pool
        ssl: true,
        idleTimeoutMillis: 5 // how long a client is allowed to remain idle before being closed
    };
    var pool = new pg.Pool(config);
}
...
async function checkAuth () {
    var connected = true;
    try{
        var subscription = require('../src/auth.json')
    ...
    var local_subscription = subscription;
    var algorithm = getALG();
    var password = getCYP();
    ...
    try{
        ...
        // console.log("Cannot get server_subscription");
        // console.log(e);
        // return;
        connected = false;
    ...
    try{
        var server_subscription_token = server_subscription.license.validation;
    }catch(e){
        connected = false;
    ...
    // console.log("server_subscription");
    // console.log(server_subscription);
    ...
    if(connected){
        var local_subscription_token = local_subscription.license.validation;

        if(server_subscription_token !== local_subscription_token){
            console.log("Subscription unmatch");

            var tempStr = "Subscription unmatch";
            var querystring_error_log = get_Querystring_error_log();
            var values = ['', '', tempStr, local_subscription_token];
            var result  = (await query_with_parameter(querystring_error_log, values));
            console.log(result);
        ...
        try{
            // fs.writeFile('../lib/auth.json', JSON.stringify(server_subscription), 'utf8', callback);
            server_subscription.license.validation = encrypt(server_subscription.license.validation, algorithm, password, crypto)
            fs.writeFile('../src/auth.json', JSON.stringify(server_subscription), function(err) {
                if(err) {
        ...
        try{
            var local_subscription_expiredate =  new Date(decrypt(local_subscription.expiry, algorithm, password, crypto));
        }catch(e){
            var tempStr = "Cannot decrypt local_subscription_expiredate";
            var querystring_error_log = get_Querystring_error_log();
            var values = ['', '', tempStr, e.toString()];
            var result  = (await query_with_parameter(querystring_error_log, values));
            console.log(result);
